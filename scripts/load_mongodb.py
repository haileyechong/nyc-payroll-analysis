...
employee_map = {
    (
        e.get("first_name"),
        e.get("last_name"),
        e.get("middle_initial"),
        e.get("agency_start_date")
    ): e["_id"]
...
db["payroll_records"].create_index("fiscal_year")
db["payroll_records"].create_index("employee_id")
db["payroll_records"].create_index("agency_snapshot.agency_name")
db["payroll_records"].create_index("title_snapshot.title_description")
db["payroll_records"].create_index("compensation.total_overtime_paid")
db["payroll_records"].create_index("compensation.regular_gross_paid")
db["payroll_records"].create_index("compensation.overtime_hours")

print(f"Loaded {len(payroll_records)} records across fiscal years: {sorted(payroll_data['fiscal_year'].unique())}")

print("MongoDB payroll_records count:", db["payroll_records"].count_documents({}))
print("Missing employee_id count:", db["payroll_records"].count_documents({"employee_id": None}))